...
(defclass double-colon-access-rule (base:rule)
  ((skip-test-files
    :initarg :skip-test-files
    :initform t
    :reader skip-test-files-p
    :type boolean
    :documentation "When T (default), skip files whose defpackage uses a test framework."))
  (:default-initargs
...
for white-box testing. Set :skip-test-files nil to check even in test files."))
...
Skips files that use a known test framework when skip-test-files is enabled."
  (unless (and (skip-test-files-p rule)
